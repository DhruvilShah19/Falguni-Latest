...
export default function DeliveryAddressInput({ onDeliveryCalculated, defaultAddress }: DeliveryAddressInputProps) {
  ...
  const handleSelect = async (suggestion: google.maps.places.AutocompleteSuggestion) => {
    ...
    try {
      ...
      await (place.fetchFields as any)({ fields: ['formattedAddress', 'location'], sessionToken: sessionTokenRef.current ?? undefined });
  ...
  return (
    <div className="w-full mt-4 bg-white border border-[var(--color-border)] p-4 md:p-6 rounded-[20px] shadow-sm relative overflow-hidden">
      <h3 className="text-[var(--color-primary)] font-bold text-xs md:text-sm uppercase tracking-widest mb-4 flex items-center gap-2">
        <Navigation size={16} /> Delivery Destination
      </h3>

      <div className="relative z-10">
        <div className="relative">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-fg-muted)]" />
          <input
            type="text"
            value={value}
            ...
            className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] focus:border-[var(--color-primary)] rounded-xl py-3 md:py-4 pl-11 pr-4 text-[var(--color-fg)] placeholder-[var(--color-fg-muted)]/60 text-sm outline-none transition-colors"
          />
          {calculating && <Loader2 size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--color-primary)] animate-spin" />}
        </div>

        {/* Dropdown Suggestions */}
        {suggestions.length > 0 && (
          <ul className="absolute z-50 w-full mt-2 bg-white border border-[var(--color-border)] rounded-xl overflow-hidden shadow-xl max-h-60 overflow-y-auto custom-scrollbar">
            {suggestions.map((suggestion) => (
              <li
                key={suggestion.placePrediction!.placeId}
                onClick={() => handleSelect(suggestion)}
                className="px-4 py-3 cursor-pointer hover:bg-[#F5EBE1] border-b border-[var(--color-border)] last:border-0 flex items-start gap-3 transition-colors"
              >
                <MapPin size={16} className="text-[var(--color-primary)] mt-0.5 flex-shrink-0" />
                <span className="text-[var(--color-fg)] text-xs md:text-sm leading-snug">{suggestion.placePrediction!.text.text}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      {activeDetails && (
        <div className="mt-5 animate-fade-up">
          <div className="bg-[var(--color-bg)] border border-[var(--color-border)] rounded-[20px] md:rounded-[24px] p-4 md:p-6 flex flex-col items-center transition-all">

            {/* Info Box (Full Width) */}
            <div className="w-full max-w-lg flex flex-col justify-center py-2 md:py-4 mx-auto">

              <div className="flex flex-col gap-1 mb-5">
                <span className="inline-block w-max text-[var(--color-primary)] font-black text-[10px] uppercase tracking-widest bg-[#F5EBE1] border border-[var(--color-border)] px-2.5 py-1 rounded-md mb-2 shadow-xs">
                  {activeDetails.tier} Delivery
                </span>
                {activeDetails.tier === 'Hyperlocal' && (
                  <span className="inline-flex items-center gap-1.5 text-[var(--color-fg-muted)] text-[10px] md:text-xs font-medium">
                    <Clock size={11} className="text-[var(--color-primary)]/70 flex-shrink-0" />
                    Delivery hours: 11 AM – 8 PM
                  </span>
                )}
                <p className="text-[var(--color-fg-muted)] text-xs md:text-sm font-medium leading-relaxed pr-4 mt-2">
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="bg-white border border-[var(--color-border)] rounded-xl p-3 flex flex-col justify-center">
                  <div className="text-[var(--color-fg-muted)] text-[9px] uppercase tracking-widest font-bold mb-1 flex items-center gap-1.5"><Navigation size={10} className="text-[var(--color-primary)]" /> Distance</div>
                  <div className="text-[var(--color-fg)] font-medium text-xs">{activeDetails.distanceText}</div>
                </div>
                <div className="bg-white border border-[var(--color-border)] rounded-xl p-3 flex flex-col justify-center">
              ...
              <p className="flex items-start gap-1.5 text-[var(--color-fg-muted)]/70 text-[9px] leading-relaxed mb-5 -mt-2">
              ...
              <div className="flex items-end justify-between mt-4">
                <div className="flex flex-col">
                  <span className="text-[var(--color-fg-muted)] text-[10px] uppercase tracking-widest font-bold mb-1">Delivery Fee</span>
                ...
                {activeDetails.fee === 0 ? (
                  <span className="text-emerald-700 font-black text-2xl tracking-tight uppercase">Free</span>
                ) : (
                  <span className="text-[var(--color-primary)] font-black text-2xl tracking-tight">₹{activeDetails.fee}</span>
                )}
              </div>

              {/* Free Shipping Call to Action */}
              <div className="mt-5 bg-[#F5EBE1] border border-[var(--color-border)] rounded-xl p-4 flex flex-col gap-3 relative overflow-hidden group shadow-xs">
                {activeDetails.fee === 0 ? (
                  <div className="flex items-center gap-3 text-emerald-800 z-10 relative">
                    <CheckCircle2 size={20} className="flex-shrink-0 text-emerald-700" />
                    <span className="text-sm font-black uppercase tracking-widest">You've unlocked Free Delivery!</span>
                  </div>
                ) : (
                  <div className="z-10 relative">
                    ...
                    {/* Progress Bar */}
                    <div className="w-full bg-white h-2.5 rounded-full overflow-hidden mt-2 border border-[var(--color-border)] shadow-inner">
                      <div
